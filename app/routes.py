...
META_PHRASES = [
    ...
    "here is a clear and concise summary", "here’s a clear and concise summary",
    "this section discusses", "the above text says",
...
async def safe_chat(messages, temperature=0.2, max_completion_tokens=300) -> str:
...
def extract_text_from_pdf(pdf_bytes: bytes) -> str:
    ...
    try:
        ...
        text = "\n".join(pages)
        # normalize hyphenation & whitespace
        text = re.sub(r"-\n", "", text)
        text = re.sub(r"\n{2,}", "\n\n", text)
...
def split_by_headings(text: str):
    ...
    if not idxs:
        # fallback to soft chunks to avoid mid-sentence cuts
...
def soft_chunks(text: str, chunk_size=3500):
...
def ensure_liability_complete(section_title: str, section_summary: str) -> str:
    """If this is Liability and it ends abruptly, nudge a safe completion."""
    if "liability" in section_title.lower():
        # If it ends like 'at their own free will and' → add 'risk.'
...
async def summarize_pdf(file: UploadFile = File(...)):
    try:
        ...
        if not text:
            raise HTTPException(status_code=400, detail="No extractable text. Is this a scanned PDF without OCR?")
        ...
        for idx, section in enumerate(sections):
            ...
            messages = [
                {"role": "system", "content":
                    ...
                    "Do NOT include meta phrases like 'here is a summary'. Keep bullets tight where helpful."},
        ...
        # Short reducer: one polished executive summary (captures intro tone + flexibility)
        reducer_prompt = (
            "Create a professional **Executive Summary (150–220 words)** of the contract from the following section summaries.\n"
